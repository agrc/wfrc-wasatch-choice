/* NOTE: This was copied from @ugrc/layer-selector on 8/25/2023
I did this because it did not support switching out the in the way that this project does.

I didn't think that it was worth complicating the shared package
with all of the hacks that are required. If we have a few more projects
come up that need this functionality, we could look to incorporate it.
*/
...
const LayerSelector = (props) => {
  ...
  useEffect(() => {
    setManagedLayers((current) => {
      ...
      const managedLayersDraft = { ...current };
      ...
      layerItems.forEach((layerItem) => {
        ...
        managedLayersDraft[layerItem.id].layer.when('loaded', () => {
          const currentScale =
            managedLayersDraft[layerItem.id].layer.tileInfo.lods[
              props.view.zoom
            ].scale;
      ...
      return managedLayersDraft;
    });
  }, [layers, props.view, props.view.map, props.visible]);
  ...
  useEffect(() => {
    ...
    if (props.view && props.visible) {
      props.view.ui.add(selectorNode.current, {
        position: props.position,
        index: props.index,
      });
      if (props.view.map.basemap?.id !== 'layer-selector') {
        props.view.map.basemap = new Basemap({
          id: 'layer-selector',
        });
      }
    } else {
      props.view.ui.remove(selectorNode.current);
  ...
  return (
    <div id="layer-selector" ref={selectorNode}>
      <ConditionalWrapper
